...
const generateVariableColors = (theme) =>
{
    ...
    }) =>
    {
        if ( opacityValue )
        {
            return `rgba(var(--cipafacil-${name}-rgb), ${opacityValue})`;
        }
        if ( opacityVariable )
        {
            return `rgba(var(--cipafacil-${name}-rgb), var(${opacityVariable}, 1))`;
        }
        return `rgb(var(--cipafacil-${name}-rgb))`;
    };
...
// -----------------------------------------------------------------------------------------------------
// @ CIPAFACIL TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
const theming = plugin.withOptions((options) => ({
    ...
    }) =>
    {
        ...
        const mapVariableColors = _.fromPairs(_.map(options.themes, (theme, themeName) => [
            ...
            ))), (value, key) => [[`--cipafacil-${e(key)}`, value], [`--cipafacil-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
        ]));
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`cipafacil.customProps.background.${colorScheme}`);
            const foreground = theme(`cipafacil.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--cipafacil-${e(key)}`, value], [`--cipafacil-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--cipafacil-${e(key)}`, value], [`--cipafacil-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
